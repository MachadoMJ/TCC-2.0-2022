...
//atualizar categoria 
  if ( isset($_POST['UpSub']) ) {
    ...
    //atualizar categoria
    $sql = "exec PRC_Update_category '$name', $Id ";

    $stmt = sqlsrv_query($conn, $sql);
    if( $stmt === false) { echo 'falha ao atualizar Categoria'; }

    //inserir Log
    $sqlL = " insert into log.tmp_log (Client,action, calendar)
              values ($_SESSION[IdUser], 'update Category [$Id]', 'Category')";
      
    $stmt = sqlsrv_query($conn, $sqlL);
    if( $stmt === false) { echo 'falha ao cadastrar categoria Log'; }

  }

//Trazendo uma caterogira
  $sql = "SELECT NCategory FROM VW_TableCategory where IdCategory = $_GET[IdCat]";
  $stmt = sqlsrv_query($conn, $sql);

  if(sqlsrv_fetch( $stmt )) {

    $nameC = sqlsrv_get_field( $stmt, 0);
  }
        ...
        <div class="row mb-2">
          ...
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              ...
              <li class="breadcrumb-item active">Atualizar Categoria</li>
            </ol>
          </div><!-- /.col -->
        </div><!-- /.row -->
      </div><!-- /.container-fluid -->
              ...
              <div class="card-body">
                <form method="post">
                  ...
                  <div class="form-group">
                    ...
                    <input type="text" class="form-control" placeholder="<?php echo $nameC; ?>" name="Name">
                  </div>
